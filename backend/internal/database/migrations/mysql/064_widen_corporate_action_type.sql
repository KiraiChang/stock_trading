...
-- +goose Up
ALTER TABLE corporate_actions MODIFY action_type VARCHAR(32) NOT NULL;

-- +goose Down
-- 回滾前必須先清掉超過 16 字元的資料，否則轉型會失敗。
DELETE FROM corporate_actions WHERE CHAR_LENGTH(action_type) > 16;
ALTER TABLE corporate_actions MODIFY action_type VARCHAR(16) NOT NULL;
