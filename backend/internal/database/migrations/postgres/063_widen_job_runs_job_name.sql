...
-- +goose Up
ALTER TABLE job_runs ALTER COLUMN job_name TYPE VARCHAR(64);

-- +goose Down
-- 回滾前必須先清掉超過 20 字元的資料，否則轉型會失敗。
DELETE FROM job_runs WHERE LENGTH(job_name) > 20;
ALTER TABLE job_runs ALTER COLUMN job_name TYPE VARCHAR(20);
