...
ALTER TABLE corporate_actions
    ADD COLUMN volume_factor REAL NOT NULL DEFAULT 1;
ALTER TABLE candles
    ADD COLUMN vol_factor REAL NOT NULL DEFAULT 1;
...
-- sqlite 不支援對既有表 ADD CONSTRAINT（要整表重建）。volume_factor > 0 由寫入端
-- （market/yahoo_dividend.go 與 finmind.go）保證；postgres／mysql 另有 DB 層約束。

-- +goose Down
ALTER TABLE candles DROP COLUMN vol_factor;
ALTER TABLE corporate_actions DROP COLUMN volume_factor;
