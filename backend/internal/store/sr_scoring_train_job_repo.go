...
type SRScoringTrainJobRepo interface {
	...
	MarkDone(ctx context.Context, jobID string, rows, sources int, metrics RawJSON, modelPath, modelVersion string) error
...
const srScoringTrainJobColumns = `id, job_id, status, symbols, timeframe, fetch_limit, model_type,
	rows, sources, metrics, model_path, model_version, error, started_at, finished_at, created_at`
...
func (r *srScoringTrainJobRepo) MarkDone(ctx context.Context, jobID string, rows, sources int, metrics RawJSON, modelPath, modelVersion string) error {
	_, err := r.db.ExecContext(ctx, r.db.Rebind(`
		UPDATE sr_scoring_train_jobs
		SET status='done', rows=?, sources=?, metrics=?, model_path=?, model_version=?, finished_at=CURRENT_TIMESTAMP
		WHERE job_id=?
	`), rows, sources, metrics, modelPath, modelVersion, jobID)
	return err
}
