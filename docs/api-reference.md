...
的驗證母體得先有辦法補跑；排程漏跑時也只有這裡能補。

**兩個時段共用一個執行所有權**（2026-08-31 起）。任一輪在跑時，另一輪**不論 cron 或
手動**都不會執行——這台 host 只有 2GiB，逐檔的峰值等同使用者手動點一次分析，
並行兩輪比少一輪糟得多。

⚠️ **上面說的是執行層。資料層的時段冪等（那張跳過條件表）兩輪規則刻意不同，兩者不要混談。**

所有權是**同步**取得的，所以回應碼是可信的答案而不是猜的：
202 代表真的啟動了、409 代表真的被擋住。
...
**Response（409 Conflict）**——另一輪仍在執行：
```json
{
  "error": "sr analysis already running",
  "running_job": "sr_analysis_chip"
}
```

`running_job` 是**目前持有所有權**的那一輪，可能與你請求的那一輪不同。
