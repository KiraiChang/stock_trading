...
**live 啟用（2026-09-08 10:57）**：`DELISTING_ENABLED=true` 確認已進容器，
migration 76 已套用，兩張新表與 `stock_symbols` 的兩個新欄位、FK／CHECK／UNIQUE 全部就位，
新端點回 401（存在且受保護），排程頁顯示 `尚未執行`（＝**已註冊**、未跑過）。
**首輪是 2026-09-09 07:00**，20 個交易日的觀察期由那一輪起算（觀察什麼見 `todo.md` T-071）。
⚠️ 首輪之前那列會標「已延遲未執行」——stale 門檻 26 小時而它從未執行過，是正常的。
...
##### 民國日期的解析是嚴格的：年、月、日三個維度都有守門

`parseROCDate`（`market/exchange_reference.go`，TWSE 年度日曆與終止上市名單共用）
**不接受任何會被靜默正規化的輸入**：

| 維度 | 守門 | 少了會怎樣 |
|---|---|---|
| 月／日 | `newStrictDate` 建完日期後**反向檢查**年月日與輸入一致 | `115/02/31` 被 `time.Date` 靜默轉成 3/3，不報錯 |
| **年** | `y <= 0` 直接回錯（民國元年 = 西元 1912） | `0/01/01` 算成 1911、`-5/01/01` 算成 1906 |

⚠️ **兩者是同一類問題**：**不存在的日期被靜默轉成一個合法日期**——看起來完全正常，
卻讓整條比對建立在一個不存在的日子上。年份那條 2026-09-08 才補
（原記於 `issue.md` I-109，已收斂）；月／日那條更早就有。
