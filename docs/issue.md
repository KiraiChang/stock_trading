...
- **移除條目前要先反轉依賴。** 主題文件與程式碼註解常寫「見 issue.md I-0xx」，
  ...
  **本節自己會出現在輸出裡**（上面提到 I-040 / I-056 / I-069 / I-070～I-072 / I-076 /
  ...
  本檔現有的 I-100 / I-103 / I-104 / I-105 / I-106 / I-107 / I-108 / I-112、已收斂的 I-102 與 I-109～I-111、
  以及下一個可用的 I-113），
  那是預期的，不是殘留。
...
### I-112：`corporate_action_sync` 的逐檔失敗不寫進 `error`，排程頁只看得到 `partial`

| 欄位 | 內容 |
|---|---|
| 狀態 | 待修復 |
| 嚴重度 | 低（狀態正確、只是看不到成因；原因查得到，在 log 裡） |
| 分類 | Go / 排程 / 可觀測性 |
| 發現日期 | 2026-09-08（live 觀察時順帶發現；那次觀察原記於 `todo.md` T-070，已收斂，結論在 `architecture.md`「live 運作觀察：四個交易日」） |
| 位置 | `backend/internal/scheduler/scheduler.go` 的 `RunCorporateActionSync` |

2026-09-03 06:30 那輪：`status=partial`、`symbols_total=175`、`symbols_failed=1`、
**`error` 是空字串**。

**這是設計如此，不是 bug**——`errParts` 只收三種情況：整體性錯誤（目前只有 ctx 逾時／
取消）、**沒有整體錯誤卻少跑了幾檔**時的「N 檔未處理」（防禦性分支，見下表）、
以及 watchlist 讀取失敗。程式註解寫得很清楚：「個別標的失敗已在 Adjuster 內記錄並跳過」。

**但它與 T-071 定案的原則不一致**：`partial` 也要寫得出原因，否則
`GET /scheduler/status` 只看得到 `partial`、看不到為什麼，得有人去翻 log 才知道
是哪一檔、什麼原因。

⚠️ **這兩種情況其實分辨得出來**（2026-09-08 review 修正——前一版寫成「無法分辨」，那是錯的）：

| 情況 | `error` | 走哪個分支 |
|---|---|---|
| ctx 逾時／取消 | `corporate_action_sync:<reason>`（`joberr` 封閉值域） | `syncErr != nil` |
| `processed < planned` **但沒有整體錯誤** | `N 檔未處理` | `else if skipped > 0` ← **防禦性分支** |
| **一般的逐檔失敗** | **空字串** | 兩個分支都不進 ← 缺口在這裡 |

⚠️ **第二列不是「逾時」**（2026-09-08 review 修正——前一版寫成逾時，那是錯的）：
正常的 ctx 逾時會讓 `SyncPerSymbolEvents` 回傳 `syncErr`，因此走**第一列**；
`N 檔未處理` 掛在 `else if`，只有「沒回傳整體錯誤卻少跑了幾檔」時才寫得到。

所以真正的缺口是**普通逐檔失敗看不到失敗階段與安全 reason**：
`symbols_failed > 0` 但 `error` 空白，排程頁上那個 `partial` 沒有任何線索。

#### 處置（未實作）

`SyncPerSymbolEvents` 回傳的 `failed` 只有數量，沒有成因。最小的修法是讓它同時回傳
**失敗檔數的 reason 統計**（`joberr` 的封閉值域，例如 `upstream_error:2`），
由 `RunCorporateActionSync` 併進 `errParts`。

⛔ **不要回傳原始 error 或 symbol 清單**：`job_runs.error` 是使用者可見面
（I-104 的契約），只能寫 stage 前綴與封閉值域的 reason code。

ℹ️ 相關但不同的一筆：I-104 處理的是「其餘排程**寫入原始錯誤**」，這一筆是
「該寫原因卻什麼都沒寫」。
