...
- **下一個新編號從 `I-094` 起算。**（I-081～I-083 於 2026-08-21 發出，I-084～I-087 於 2026-08-24 發出，I-088～I-092 於 2026-08-25 發出，I-093 於 2026-08-26 發出並於同日收斂。）
  **發出新編號時記得把這一行一起往前推**——上一次就是漏了這步，I-089 發出去之後
  這裡還寫著「從 I-089 起算」，差一點又重用一次（I-070 已經發生過）。
  檔案裡看得到的最大是 I-092，但被移除的條目
...
- **移除條目前要先反轉依賴。** 主題文件與程式碼註解常寫「見 issue.md I-0xx」，
  ...
  **本節自己會出現在輸出裡**（上面提到 I-040 / I-056 / I-069 / I-070～I-072 / I-076 /
  I-083 / I-084 / I-086～I-090 / I-093 與下一個可用的 I-094），那是預期的，不是殘留。
...
### I-091：`2867` 的日 K 停在 2026-08-19，但沒有任何機制會讓它浮出來

| 欄位 | 內容 |
|---|---|
| 狀態 | **待決策**（需要先確認上游狀態，再決定退池或加自動機制） |
| 嚴重度 | 低（單一標的；但會讓 `evaluation_universe_sync` 的 `partial` 變成常態噪音） |
| 分類 | 資料 / 評估標的池 |
| 發現日期 | 2026-08-25 |
| 來源 | live 評估標的池同步中斷後的唯讀盤點（2026-08-25）。**觸發那次盤點的 I-090 已收斂，本筆仍未決策** |

盤點 `evaluation_universe` 的 135 檔時，其餘標的的最後一根日 K 都在 08-24 或 08-25，
只有 `2867` 停在 **2026-08-19**，且當天量能從常態的 1,200～2,200 萬跳到 **7,621 萬**：

```
2867 | 2026-08-19 |  9.70 | 76212030
2867 | 2026-08-18 |  9.82 | 22793640
2867 | 2026-08-17 |  9.71 | 15248361
```

這是典型的下市／合併前最後爆量。**這與 08-25 的中斷無關**——`2867` 排在池內第 60 位，
當天本來就沒輪到；它是在那之前就已經停止更新。

**為什麼要記**：若上游確實不再供應這檔，它會在**每一輪**同步都失敗一檔，
把 `evaluation_universe_sync` 的常態狀態壓成 `partial`。一個永遠是 `partial` 的 job
等於沒有狀態——這正是 [`api-reference.md`](./api-reference.md) 反覆警告的
「訓練使用者忽略旗標」。

**可能作法（擇一，實作前要先確認）**：

* **(a) 確認後手動退池**：`POST /api/v1/evaluation-universe/{symbol}/active` 設 `false`。
  一行動作，但下一檔下市時會重演。
* **(b) 連續 N 輪失敗自動退池並記 Error log**：通用解，但需要在 `evaluation_universe`
  加失敗計數欄位（＝ migration ×3），而且「自動把標的移出研究母體」本身要謹慎——
  上游暫時性故障不該造成永久退池。

**建議先做 (a)**，等第二檔出現同樣狀況再評估 (b)。在那之前這筆的成本是每輪一檔失敗。
