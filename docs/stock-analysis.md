...
各自的觸及時間（`hit_at`）都獨立保留，供使用者比較先後。

驗證另外輸出一個保守研判 `resolution`（`first_exit`：`STOP_LOSS` /
`TAKE_PROFIT` / `NONE`）：取**最早觸及**的停損 vs 最早觸及的停利比較時間，較早
的那一方即研判先出場。若兩者落在**同一根K棒**（`same_bar_tie = true`），因為
日K下 High/Low 誰先發生無法從 OHLC 還原，保守假設**停損先出場**（跟 SR Zone
`labeling.py` 的 tie-break 一致）。這是啟發式而非真實盤中順序，`hit_at` 原始值
仍保留供人工核對。
...
- 停利的 `next_level` 若支撐/壓力清單在那個方向上沒有更遠的 level，會是
  `null`，不會硬湊一個數字。
- 驗證的 `resolution` 會給出保守研判（同一根K棒同時觸及時假設停損先出場），
  但這是日K下的啟發式、非真實盤中順序；要精確判斷仍需比較各自的 `hit_at`
  原始值（同根同時觸及時兩者時間戳相同，日K下本來就無法再細分先後）。
