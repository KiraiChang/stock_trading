...
| 項目 | 說明 |
|---|---|
| **除權息的增量更新** | 目前每次全抓。逐檔查詢受 `yahoo.rate_limit` 節制，28 檔約 1.5 分鐘，**擴到 1,900 檔是數小時**。這是 T-040 擴標的池的**前置條件**，不是日後優化 |
| **模型用未還原資料訓練** | `sr_scoring_v4.joblib` 的訓練資料是未還原價。反向調整只影響跨越事件的窗口、且特徵多為相對值，影響應該有限——但**這是推論不是證據**。要跑一次 evaluation 對照特徵分布，有實質偏移就得重訓 |
...
| 減資／合併未涵蓋 | 見 [`issue.md`](./issue.md) I-069。需要先找到資料源 |
