...
**交付 Step 2 的早期證據**：40 檔的 ATR% 分佈顯示 **LOW bucket 全部是債券 ETF**，
而 0.76%（最高的債券 ETF）到 1.57%（最低的股票 `3045` 台灣大）之間**完全是空白**。
最低波動的個股仍是 NORMAL。這比 2026-08-06 只有 `0050` 3.25% 的證據更強：
Step 2 的結論恐怕不只是「門檻要重定」，而是**台股個股端根本不存在 LOW 這個群體**，
LOW bucket 若要有資料就只能由債券 ETF 填——而那正是計畫書判定「拿來調參反而有害」的情況。
