...
### T-035：semantic pipeline 收尾清理（position state 冗餘與 reason code 對齊）
...
| 分類 | Backend / 重構 |
| 建立日期 | 2026-07-23 |
| 來源 | T-034-P3 issue 修正 review 發現 |

T-034-P3 已把 `position_gate_state` 統一為 `semantic_pipeline.action_state`，但留下兩個
不影響正確性的收尾項目：

- `_position_action_condition`（`decision_engine.py:167-171`）的
  `semantic_pipeline.get("action_state") or derived_view.get("position_gate_state") or "NORMAL"`
  現在兩個來源都等於 `action_state`，已冗餘；可簡化為只讀 `action_state`。另 fallback 值不一致
  （此處 `"NORMAL"`、`_decision_derived_view` 為 `"WATCH"`），雖屬死路徑仍宜統一。
- `_decision_derived_view` 各分支仍輸出如 `POSITION_RESISTANCE_OVERHEAD` 的 `position_reason_codes`，
  而 `position_gate_state` 已改為 `action_state`（可能是 `WATCH`）。兩者不矛盾但語意層級不同，
  若要讓 reason code 與 state 完全對齊可再收斂。
...
- **T-034：對外標籤統一由 Event Lifecycle 推導**（2026-07-23 結案）。P0–P2 已把
  ...
  「lifecycle family 規則」，不再於本清單追蹤。P3 後已新增
  `decision_derived_view.semantic_pipeline`，將語意鏈收斂為
  `Event -> Lifecycle -> Market State -> Bias -> Action -> Entry`，並讓
  `market_bias`、`position_action_condition.state`、`final_entry_permission.state`
  由此 contract 推導；API 範例與規格文件已同步更新。
