#!/usr/bin/env bash
# MySQL migration 實跑驗證：起一個專用的 MySQL，跑完整 goose up → 驗 schema → down 到 0，
# 最後收乾淨。補上 docs/issue.md I-054 的缺口——mysql 那份 migration 在此之前從未被執行過。
#
# 用法：
#   scripts/test-mysql-migrations.sh              # 完整驗證（跑完會 down -v）
#   KEEP_UP=1 scripts/test-mysql-migrations.sh    # 跑完保留 MySQL，方便手動連進去看 schema
#
# 可覆寫的環境變數：
...
#   WAIT_SECONDS  等 MySQL healthy 的上限秒數（預設 120）
#   KEEP_UP     設 1 則跑完不收 MySQL（預設 0）
...
#   - 每次都從空 DB 開始：開頭與結尾都 `down -v`。這是**專用 project**（stock_trading_mysql），
...
NETWORK_NAME=""
MYSQL_DSN="trading:trading@tcp(mysql:3306)/trading?parseTime=true"
...
cleanup() {
  ...
  if [ "$KEEP_UP" = "1" ]; then
    echo "==> KEEP_UP=1，保留 MySQL（手動收：docker compose -f $COMPOSE_FILE down -v）"
  else
    echo "==> 收掉 MySQL（含 volume，下次從空 DB 開始）"
...
# MySQL 調瘦後實測約 200~300MB，加上執行 binary 的 container 與 dockerd 本身，
...
==> 記憶體不足，中止。
    ...
    不要改大 MySQL 的 mem_limit——那是 cgroup 上限不是預留，只會讓 host OOM killer 改砍呼叫端。
EOF
    exit 1
  fi
fi

# ── 階段 A：編譯測試執行檔（MySQL 還沒起來，記憶體全給編譯器）────────────
...
docker run --rm \
  ...
  -v "$BIN_DIR":/out \
  "$GO_IMAGE" \
  go test -c -o /out/migrate-mysql.test ./internal/database

# ── 階段 B：起 MySQL（編譯器已退場）──────────────────────────────────
echo "==> [2/4] 啟動 MySQL：$COMPOSE_FILE"
compose down -v --remove-orphans >/dev/null 2>&1 || true
compose up -d

echo "==> 等 MySQL healthy（上限 ${WAIT_SECONDS}s）"
...
while true; do
  ...
  if (( SECONDS >= deadline )); then
    echo "ERROR: MySQL 在 ${WAIT_SECONDS}s 內沒有變成 healthy（目前 ${health:-unknown}）" >&2
...
if [ -z "$NETWORK_NAME" ]; then
  echo "ERROR: 取不到 mysql container 的網路名稱" >&2
  exit 1
fi
...
docker run --rm \
  ...
  --memory-swap=128m \
  --pids-limit=100 \
  -e MYSQL_MIGRATION_DSN="$MYSQL_DSN" \
  -v "$BIN_DIR":/out:ro \
  "$RUN_IMAGE" \
  /out/migrate-mysql.test -test.v -test.run 'TestMySQLMigrations'

# ── 階段 D：收尾由 trap 處理 ────────────────────────────────────────
echo "==> [4/4] mysql migration 驗證通過"
